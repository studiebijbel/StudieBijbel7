 ...
 * @type {String}
 *
 * @properties={typeid:35,uuid:"5C29692B-D1C8-4EF9-A6CB-F21C4118FBDC"}
 */
var msgText = "";
	...
	for (i = 0; i < buttonArray.length; i++) {
		...
		for (j = 0; j < buttonArray[i].length; j++) {
			for (k = 0; k < _aGuesstimator.length; k++) {
				/** @type {RegExp} */
				var regex = _aGuesstimator[k][0];
				if (regex.test(buttonArray[i][j])) {
...
/**
 * @properties={typeid:24,uuid:"FF8EDFFF-85CF-48F0-9C13-B3A311AF379D"}
 */
function isMac() {
	return application.getOSName().indexOf("Mac") > -1;
}
